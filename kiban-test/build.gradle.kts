...
kotlin {
    jvm()
    androidLibrary {
        namespace = "nl.bijdorpstudio.kiban.test"
    ...
    sourceSets {
        // `api` (not `implementation`): consumers pull in `kiban-test` as their sole
        // test-time dependency and expect `Iban` et al. to come along with it, the same
        // way a `java-test-fixtures` consumer gets the main module's classes for free.
...
mavenPublishing {
    ...
    coordinates(group.toString(), "kiban-test", version.toString())

    pom {
        name = "Kiban Test Fixtures"
        description = "Test helpers for consumers of the Kotlin Multiplatform IBAN library."
        inceptionYear = "2025"
